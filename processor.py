...
# Holds the start time and type of sustain event
class SustainAdapter:
    def __init__(self, time, type):
        self.start =  time
        self.type = type

# Manages sustain up and down events - start, end and affected notes
...
# Stores the type and value of an event, convert to and from int
class Event:
...
# Divides each note into note_on and node_off events, returns new list. Used for encoding
...
# Merges note_on and node_off into Note object. Used for decoding
    ...
    for snote in snote_sequence:
        ...
        if snote.type == 'note_on':
            ...
            try:
                ...
                # If there is a missing note_on event for a note_off event with the same pitch
                print('info removed pitch: {}'.format(snote.value))
    return result_array

# Converts SplitNode to list of Event objects
...
# Converts list of Event objects to SplitNote objects
...
# Proccesses sustain control changes
...
# Processes notes based on sustain adapters, return final note_stream
...
# Encodes file, extracting control changes and notes from each instrument
...
def decode_midi(idx_array, file_path=None):
    ...
    instument = pretty_midi.Instrument(1, False, "test")
