...
criterion = criteria.SmoothCrossEntropyLoss()   # Prevents Overfitting
...
with torch.no_grad():
    for b in range(len(dataset.file_dict["train"]) // B):
        x, y = dataset.slide_seq2seq_batch(B, L, 'train')
        x = torch.from_numpy(x).contiguous().to(device, dtype=torch.int)
        y = torch.from_numpy(y).contiguous().to(device, dtype=torch.int)
...
print(f"Train Accuracy: {average_accuracy:.4f}")
